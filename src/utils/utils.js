...
export function useInitParse() {
    return new Promise(async (resolve, reject) => {
        console.log("\nINITIATING PARSE")
        let parse_app_id = localStorage.getItem('parse_app_id') ? localStorage.getItem('parse_app_id') : "default";
        let parse_url = "/parse"
...
export function useInitializeDefaultUser() {
    ...
    if (savedUser) {
        ...
        localStorage.setItem('user_default-user', JSON.stringify(defaultUser))
        // 初始化用户列表
        const users = [{ id: 'default-user', name: 'Default User' }]
...
export function useGetTimeZone() {
    ...
    timeZoneTrade.value = currentUser.value && currentUser.value.hasOwnProperty("timeZone") ? currentUser.value.timeZone : 'America/New_York'
...
export const useGetLegacy = async () => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (savedUser) {
            ...
            let currentLegacy = userData.legacy
...
export const useUpdateLegacy = async (param1) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (savedUser) {
            ...
            // 更新本地存储中的用户数据
            userData.legacy = currentLegacy
            localStorage.setItem('currentUser', JSON.stringify(userData))
            // 更新当前用户对象
            currentUser.value = userData
            
            console.log(' -> Saved/Updated legacy')
            resolve()
...
export const useGetAPIS = async () => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (savedUser) {
            ...
            if (userData.apis != undefined) {
                for (let index = 0; index < userData.apis.length; index++) {
                    const element = userData.apis[index];
                    apis.push(element)
                }
            }
...
export const useGetLayoutStyle = async () => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (savedUser) {
            ...
            if (userData.layoutStyle != undefined) {
                for (let index = 0; index < userData.layoutStyle.length; index++) {
                    const element = userData.layoutStyle[index];
