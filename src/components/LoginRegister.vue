...
<template>
  <main class="container" id="registerSignup">
    <form class="text-center col-md-4 offset-md-4 mt-5" v-on:submit.prevent="pageId == 'login' ? login() : register()">
      <img class="mb-4" src="">
      <h1 class="h3 mb-3 fw-normal">{{ pageId == 'login' ? t('login.title') : t('register.title') }}</h1>
      ...
      <input type="password" id="inputPassword" class="mt-1 form-control" :placeholder="t('login.password')" required=""
      ...
      t('login.submit') : t('register.submit') }}<span v-if="signingUp" class="ms-2 spinner-border spinner-border-sm" role="status"
          aria-hidden="true"></span></button>
    </form>
    <div class="text-center mt-3" v-if="!registerOff"><a :href="pageId == 'login' ? '/register' : '/'"> {{ pageId == 'login' ? t('login.register') :
      t('register.login') }} page</a></div>
  </main>
</template>