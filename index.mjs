...
if (process.env.MONGO_URI) {
    ...
    // 注意：这里需要根据 Supabase 的实际配置进行调整
    const supabaseUrl = process.env.SUPABASE_URL;
    const supabaseKey = process.env.SUPABASE_KEY;
    // 提取数据库连接信息
    // 示例：supabaseUrl = "https://<project-id>.supabase.co"
    const projectId = supabaseUrl.split('.')[0].split('//')[1];
    databaseURI = `mongodb+srv://postgres:${supabaseKey}@${projectId}.supabase.co/postgres?authSource=admin`;
...
let tradenoteDatabase = process.env.TRADENOTE_DATABASE || 'postgres';

var app = express();
app.use(express.json());

// 使用 Vercel 分配的端口或默认端口
const port = process.env.PORT || process.env.TRADENOTE_PORT || 3000;
...
const startIndex = async () => {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const serv = new ParseServer({
                    ...
                    masterKey: process.env.MASTER_KEY || 'default-master-key',
                    port: port,
                ...
                console.error("Error starting Parse Server:", error.message);
                // 继续执行，不中断流程
                resolve()
            }
        })
    ...
    try {
        ParseNode.initialize(process.env.APP_ID || 'default-app-id')
        // 使用 Vercel 提供的环境变量获取部署 URL
        const vercelUrl = process.env.VERCEL_URL || `localhost:${port}`;
        ParseNode.serverURL = `http://localhost:${port}/parse`
        ParseNode.masterKey = process.env.MASTER_KEY || 'default-master-key'
    } catch (error) {
        console.error("Error initializing ParseNode:", error.message);
    }
